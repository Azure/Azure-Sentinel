# Microsoft Sentinel Solutions Packaging Tool Guidance For CCF Connector
...
4. There can be only 1 data definition file per CCF connector. If you have multiple CCF connectors then create multiple data connector definition files and specify the file paths in the data folder input file under `Data Connectors` array. 
...
GitHub keeps `EGSystemTopicName` explicitly optional with `"validations": { "required": false }`. This generates an empty default without `minLength`, allowing the first connection to create a system topic when none exists. With managed-identity delivery enabled, a supplied topic is read before updating it to enable system-assigned identity. Existing tags, attached user-assigned identities, and any already enabled system-assigned principal are preserved. The generated event subscription waits for topic preparation, the notification queue, and any newly deployed sender grant. Other connector definitions and the default delivery path remain unchanged.

The notification queue dependency uses `notificationQueueResourceId`, generated with ARM's `resourceId()` function. Do not append the queue name to its full resource type: child resource IDs must interleave type and name segments (`storageAccounts/{account}/queueServices/default/queues/{queue}`).

The upstream generator restores the original connector-role GUID formulas for all blob connectors: `guid(storageAccountName, blobContainerName)`, `guid(storageAccountName, queueName)`, and `guid(storageAccountName, dlqName)`. GitHub uses those same formulas without a connector-specific override. This preserves existing assignment IDs on rebuilds but retains the legacy limitation that changing the principal or role for the same container or queue may conflict with an existing assignment. The separate Event Grid sender grant still uses its own scope, topic principal, and role as GUID inputs.

Table discovery excludes a connector's already assigned primary table from the additional-table list. Match each candidate against that connector record's own folder and table path so a different connector's primary table is not also emitted into the first connector's content template.

Set `"eventGridUseManagedIdentity": true` on a `StorageAccountBlobContainer` poller to use the system topic's system-assigned identity for queue delivery. This generator-only boolean is removed from the emitted data connector. GitHub enables it; omitted or false values preserve the existing delivery template for other connectors.

In this mode, ARM creates or updates the system topic with a system-assigned identity. The existing-topic reference is evaluated outside the inner-scoped topic deployment so the previous identity and tags are available before the update. Topics with attached user-assigned identities retain them through `SystemAssigned, UserAssigned`; otherwise, the identity type is `SystemAssigned`. The post-deployment principal ID is used for sender access. The `eventGridRbacMode` parameter defaults to `CreateRoleAssignment`; `UseExistingRoleAssignment` skips the Event Grid sender-role deployment and relies on a pre-existing grant. Use Create when system-assigned identity is being enabled for the first time. GitHub exposes these mutually exclusive choices through the connector UI's existing `Radio` control support. Other opt-in connector definitions can use the same parameter and values.

When Create is selected, the resolved topic principal is passed to a separate nested deployment so the **Storage Queue Data Message Sender** assignment retains its deterministic name based on destination storage-account scope, principal, and role. Use account scope as specified by [Event Grid Storage Queue delivery](https://learn.microsoft.com/azure/event-grid/managed-service-identity#use-the-azure-cli---azure-storage-queue). This grants message-add access to queues in the selected account, not message-read/delete or blob access. The generated event subscription is always deployed with `deliveryWithResourceIdentity` using `SystemAssigned`; it depends on the sender deployment for Create or the topic-preparation deployment for Use existing, and always depends on the notification queue. Subscription delivery settings and filters remain connector-managed. The three Sentinel service-principal grants and their legacy naming policy remain unchanged. No deployment scripts, user-assigned identity resources, temporary storage, or container instances are required.

Upgrades from the earlier queue-scoped sender grant create a distinct account-scoped assignment; the deterministic naming formula remains scope/principal/role-based. Incremental deployment leaves the previous queue grant in place and does not modify its scope or reuse its GUID. Newly created grants can still require up to 10 minutes for RBAC propagation; ARM dependencies ensure creation order, not data-plane readiness. Retry with the same topic and identity rather than recreating them.
...
NSP is the customer's choice. Managed-identity delivery is required for subscription-based inbound NSP rules; if a perimeter is enforced, it must allow the Event Grid topic's subscription and the connector's Scuba IP ranges. The template does not change perimeter configuration. See [Enable network security](https://learn.microsoft.com/azure/sentinel/enable-storage-network-security). An existing topic name must identify a topic for the same storage source; system-assigned identity is enabled if missing without disabling an existing identity. The collector application is not migrated.
...
### Working with Single Data Definition having multiple Data Connector Poller in single file and multiple DCR in single file:
  ...
  ![Alt text](./ccfimages/multiplePollerInSingleFile.png)
...
![Alt text](./ccfimages/dcr-browser-network.png)
