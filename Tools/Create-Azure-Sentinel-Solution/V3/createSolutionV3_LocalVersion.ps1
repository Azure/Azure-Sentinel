...
Write-Host '=======Starting Package Creation using V3 tool with Local Version========='
...
function GetIncrementedVersion($version, $bumpType = "patch")
{
    ...
    try {
        [int]$majorInt = [int]$major
        [int]$minorInt = [int]$minor
        [int]$patchInt = [int]$patch
    }
    catch {
        Write-Host "Error: Cannot convert version parts to integers. Version: $version"
    ...
    switch ($bumpType.ToLower()) {
        "major" { 
            ...
            $minorInt = 0
            $patchInt = 0
            Write-Host "Incrementing MAJOR version: $version -> $majorInt.$minorInt.$patchInt"
        }
        "minor" { 
            $minorInt += 1
            $patchInt = 0
            Write-Host "Incrementing MINOR version: $version -> $majorInt.$minorInt.$patchInt"
        }
        "patch" { 
            $patchInt += 1
            Write-Host "Incrementing PATCH version: $version -> $majorInt.$minorInt.$patchInt"
        }
        default { 
            $patchInt += 1
            Write-Host "Incrementing PATCH version (default): $version -> $majorInt.$minorInt.$patchInt"
        }
    }
...
try {
    ...
    foreach ($inputFile in $(Get-ChildItem -Path "$solutionFolderBasePath\$dataFolderName\$dataFileName")) {
        ...
        #================START: IDENTIFY PACKAGE VERSION FROM LOCAL SOLUTION=============
        ...
        #================END: IDENTIFY PACKAGE VERSION FROM LOCAL SOLUTION=============
...
updateDescriptionCount $global:functionAppList.Count                           "**Function Apps:** "                       "{{FunctionAppsCount}}"             $(checkResourceCounts @($global:playbookCounter))
    ...
    Write-Host "Error occurred in catch of createSolutionV3_LocalVersion file Error details are $_" -ForegroundColor Red
}
