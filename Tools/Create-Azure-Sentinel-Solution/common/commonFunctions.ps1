...
function generateParserContent($file, $contentToImport, $contentResourceDetails) {
    ...
    elseif ($file -match "(\.json)$") {
        try {
            $jsonParserObj = ConvertFrom-Json $content -ErrorAction Stop
            # Check if this is a standalone savedSearches parser resource with a functionAlias
            if ($null -ne $jsonParserObj.properties -and $null -ne $jsonParserObj.properties.functionAlias) {
                # Map JSON parser properties to the same structure the YAML path expects,
                # so all downstream functions (getParserDetails, addTemplateSpecParserResource)
                # work without modification.
                $yaml = [PSCustomObject]@{
                    FunctionName   = $jsonParserObj.properties.functionAlias
                    FunctionQuery  = $jsonParserObj.properties.query
                    Category       = $jsonParserObj.properties.category
                    Description    = $jsonParserObj.properties.displayName
                    FunctionParams = $null
                    Function       = [PSCustomObject]@{
                        Title   = $jsonParserObj.properties.displayName
                        Version = "1.0.0"
                    }
                }
                $isyaml = $true
                Write-Host "JSON parser file detected: $file (functionAlias: $($yaml.FunctionName))"
            }
        }
        catch {
            Write-Host "Failed to deserialize JSON parser file $file" -ForegroundColor Red
        }
    }
