...
4. **KQL validation tables** — Extracted from `.script/tests/KqlvalidationsTests/CustomTables/*.json` files. These are table schema definitions used for KQL query validation in the repository's CI pipeline. Only tables not already covered by sources 1–3 are included.
