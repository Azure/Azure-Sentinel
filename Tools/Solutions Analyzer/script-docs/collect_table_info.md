...
Column-level schema for tables documented in Azure Monitor and Defender XDR reference pages. Generated by fetching rendered HTML pages from learn.microsoft.com and parsing the column tables.

| Column | Description |
|--------|-------------|
| `table_name` | Table name |
| `column_name` | Column name |
| `column_type` | Column data type (e.g., `string`, `datetime`, `dynamic`, `real`, `boolean`, `int`) |
| `description` | Column description from documentation |

This file is used by `map_solutions_connectors_tables.py` (via `--la-table-schemas-csv`) to merge documentation-sourced schemas with DCR-sourced schemas into the unified `table_schemas.csv`.

> **Note:** Column schemas are only collected when `--skip-details` is not used, since they require fetching rendered HTML pages from learn.microsoft.com. The column data is fetched separately from the raw markdown used for other table metadata because column definitions use `!INCLUDE` directives that can only be resolved from rendered pages.
