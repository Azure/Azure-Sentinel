# ESETPROTECT

## Solution Information

| Property | Value |
|----------|-------|
| **Publisher** | ESET Netherlands |
| **Support Tier** | Partner |
| **Support Link** | [https://techcenter.eset.nl/en/](https://techcenter.eset.nl/en/) |
| **Categories** | domains |
| **First Published** | 2021-10-20 |
| **Solution Folder** | [https://github.com/Azure/Azure-Sentinel/blob/master/Solutions/ESETPROTECT](https://github.com/Azure/Azure-Sentinel/blob/master/Solutions/ESETPROTECT) |\n\n## Data Connectors

This solution provides **1 data connector(s)**.

### [Deprecated] ESET PROTECT

**Publisher:** ESET

This connector gathers all events generated by ESET software through the central management solution ESET PROTECT (formerly ESET Security Management Center). This includes Anti-Virus detections, Firewall detections but also more advanced EDR detections. For a complete list of events please refer to [the documentation](https://help.eset.com/protect_admin/latest/en-US/events-exported-to-json-format.html).

**Tables Ingested:**

- `Syslog`

**Connector Definition Files:**

- [Connector_Syslog_ESETPROTECT.json](https://github.com/Azure/Azure-Sentinel/blob/master/Solutions/ESETPROTECT/Data%20Connectors/Connector_Syslog_ESETPROTECT.json)

## Tables Reference

This solution ingests data into **1 table(s)**:

| Table | Used By Connectors |
|-------|-------------------|
| `Syslog` | [Deprecated] ESET PROTECT |

---\n\n[← Back to Solutions Index](../solutions-index.md)\n