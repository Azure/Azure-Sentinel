...
namespace AzureSentinel_ManagementAPI.Bookmarks
{
    public class BookmarksController
    {
        ...
        {
            var bookmarks = Utils.LoadPayload<BookmarkPayload[]>("BookmarkPayload.json");
        ...
        /// <returns></returns>
        public async Task<string> DeleteBookmark(string bookmarkId)
        {
            try
            {
        ...
        /// <param name="bookmarkId"></param>
        /// <returns></returns>
        public async Task<string> GetBookmarkById(string bookmarkId)
        {
            try
            {
                var insId = Utils.SelectInstance(azureConfigs);
        ...
        private async Task GetBookmarksByInstance(int i)
        {
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    Utils.WriteJsonStringToFile($"GetBookmarks_{azureConfigs[i].InstanceName}.json", formattedRes, false);
