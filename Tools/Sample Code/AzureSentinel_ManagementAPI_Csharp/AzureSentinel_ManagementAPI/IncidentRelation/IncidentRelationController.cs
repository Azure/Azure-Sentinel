...
namespace AzureSentinel_ManagementAPI.IncidentRelation
{
    class IncidentRelationController
    {
        ...
        /// <param name="relationId"></param>
        /// <returns></returns>
        public async Task GetIncidentRelationByName(string incidentId, string relationId)
        {
            try
            {
                var insId = Utils.SelectInstance(azureConfigs);
        ...
        /// <param name="incidentId"></param>
        /// <returns></returns>
        public async Task GetEntitiesforIncident(string incidentId)
        {
            await GetIncidentRelationsAndEntities(incidentId, EntityKind.SecurityAlert, true);
        }
        ...
        /// <param name="incidentId"></param>
        /// <returns></returns>
        public async Task GetIncidentEntitiesbyEntityType(string incidentId)
        {
            Console.WriteLine(Utils.GetString("Select_Entity_Kind"));
            var entityKind = Utils.SelectKind<EntityKind>();
            await GetIncidentRelationsAndEntities(incidentId, entityKind, false);
        }
        ...
        /// <returns></returns>
        public async Task GetIncidentRelationsAndEntities(string incidentId, EntityKind entityKind, bool allKind)
        {
            try
            {
                var insId = Utils.SelectInstance(azureConfigs);
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    Utils.WriteJsonStringToFile($"{fileNamePrefix}_{azureConfigs[insId].InstanceName}.json",
                        serializedResources, false);
                    return;
                }
