...
namespace AzureSentinel_ManagementAPI
{
    static class Utils
    {
        ...
        public static void WriteJsonStringToFile(string fileName, string jsonData, bool format = true)
        {
            try
            {
                if (format)
                ...
                string projectPath = Path.GetDirectoryName(Path.GetDirectoryName(
        ...
        /// <param name="fileName"></param>
        /// <returns></returns>
        public static T LoadPayload<T>(string fileName)
        {
            try {
                Console.WriteLine($"Request body from {fileName}:");
                string projectPath = Path.GetDirectoryName(Path.GetDirectoryName(
                    Path.GetDirectoryName(System.IO.Directory.GetCurrentDirectory())));
                string resultFolder = "Templates";
                string filePath = Path.Combine(projectPath, resultFolder, fileName);
