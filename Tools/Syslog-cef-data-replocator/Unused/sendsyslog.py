...
"""
def get_cef_header(headers, record):
    extensions1 = {}
    values = [i.strip() for i in record.split(',')]
    cef_header_fields = ['name', 'deviceVendor', 'deviceProduct', 'signatureId', 'version', 'deviceVersion', 'severity']
    #headers_ext = [i for i in headers if i not in cef_header_fields]
    for i,field in enumerate(headers):
        if (field in cef_header_fields): 
...
def build_message_from_raw(args):
    #print ("I am here")
    try:
...
def build_message_kvpir(args, headers):
    try:
        with open(args.input_file, 'r', encoding="utf8") as file:
            lines = file.readlines()
            if len(lines) >= 1:
                for record in lines:
                    kvsearch = re.search("CEF:[0-9]\|([\w\s.\|]+)\|(.*)", record, re.IGNORECASE)
                if kvsearch:
                    cef_header = kvsearch.group(1)
                    cef_kvpairs = kvsearch.group(2)
                    extenstion_data = get_kv_pairs_csv(headers,record.strip())
                    if schemaSampledata != "NULL":
                        extenstion_data = build_custom_extension(schemaSampledata,complete_header=headers, extensions=extenstion_data)
                    syslog_message_format(args,schemaSampledata,extenstion_data)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fileformat', type=str, default='csv', help='csv or kvpair')
    parser.add_argument('--eventcount', type=int, default=2, help='Max events')
