...
## Automatic AR/CD qualification

When the user asks to test, qualify, or validate a created Analytic Rule and
Custom Detection, treat that request as one connected operation. Do not ask the
user to locate scripts, contracts, DCRs, mock folders, or report paths.

The agent must:

1. Resolve the AR/CD pair from source provenance.
2. Reuse the rule-specific `mock.json` and `scenario.json`, or invoke
   `Tools\SolutionMigration\generate_mock_scenario.py`.
3. For a registered supported Standard table, use the generated
   `ingestion-contract.yaml` and the locked workspace:
...
4. Provision or reuse only the isolated contract-specific DCE/DCR.
5. Verify acceptance, destination-table visibility, and DCR processing
   evidence independently.
6. Run the exact AR and CD queries against the same mock timestamps and unique
   scenario values. Require non-zero semantic row parity.
7. Only after row parity passes, run controlled alert parity.
   Temporary five-minute AR/CD clones are preferred for fast qualification
   when the original schedules are longer. Delete the clones after success or
   failure.
8. Update `scenario.json`, workflow state, and the versioned qualification
   report automatically.

The only normal user interruption after Qualification approval is exact-scope
write approval. Stop for manual review when mock generation reports complex,
unsupported, baseline-dependent, watchlist-dependent, or native-telemetry-only
behavior. Never turn an unsupported table into a custom lookalike table.
