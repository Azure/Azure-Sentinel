# Changelog for vimAlertEventWizCloud.yaml
...
- (2026-08-31) Initial creation of the parser
    - Normalizes and filters Wiz Cloud issues (`WizIssuesV3_CL`). Vendor/product are `Wiz`/`Cloud`, distinct from the sibling `ASimAlertEventWizDefend` (`Wiz`/`Defend`) parser for `WizDetectionsV3_CL`