        ...
        for row in csv_reader:
            if len(row) > 1:
                if len(row) >= 21:
                    ...
                    try:
                        event['DLP file label'] = row[32]
                    except IndexError:
                        pass
		    #Version 8 - The same as version 7, but adds the Isolate Action, File Action, and Warn Status fields to the Proxy log
                    try:
                        event['Isolate Action'] = row[33]
                    except IndexError:
                        pass                                                         
                    try:
                        event['File Action'] = row[34]
                    except IndexError:
                        pass
                    try:
                        event['Warn Status'] = row[35]
                    except IndexError:
                        pass  	
		    #Version 9 - The same as version 8, but adds the Forwarding Method and Producer fields to the Proxy logs.
                    try:
                        event['Forwarding Method'] = row[36]
                    except IndexError:
                        pass                                                         
                    try:
                        event['Producer'] = row[37]
                    except IndexError:
                        pass		                                    
