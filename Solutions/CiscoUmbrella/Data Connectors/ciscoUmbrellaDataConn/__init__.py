...
class UmbrellaClient:
    ...
    def _get_s3_prefix(self, aws_s3_bucket):
        aws_s3_bucket = self._normalize_aws_s3_bucket_string(aws_s3_bucket)
        logging.info(f"S3 bucket: {aws_s3_bucket}")
        tokens = aws_s3_bucket.split('/')
        logging.info(f"S3 tokens: {tokens}")
        if len(tokens) > 1:
            prefix = '/'.join(tokens[1:]) + '/'
            logging.info(f"prefix name: {prefix}")
        else:
            prefix = ''
            logging.info(f"else prefix name: {prefix}")
        return prefix
        ...
        for folder in folders:
            ...
            while True:
                ...
                if response['IsTruncated'] is True and ts_to > file_obj['LastModified']:
                    ...
                    logging.info(f"marker name: {marker}")
                else:
                    break
