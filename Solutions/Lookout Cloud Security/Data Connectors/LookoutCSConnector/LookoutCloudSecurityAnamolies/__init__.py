...
class LookOut:
    ...
    def generate_date(self):
        ...
        if past_time is not None:
            ...
            logging.info("There is no last run timestamp, trying to get Anamolies for last week.")
...
class Sentinel:
    ...
    def post_data(self, body, chunk_count):
        ...
        if (response.status_code >= 200 and response.status_code <= 299):
            logging.info("Chunk was processed{} Anomalies".format(chunk_count))
...
def main(mytimer: func.TimerRequest) -> None:
    ...
    try:
        ...
        logging.info('Total Anomalies processed successfully: {}, failed: {}. Period: {} - {}'
            .format(success_processed, fail_processed, startTime, endTime))
    except Exception as err:
      logging.error("Something wrong. Exception error text: {}".format(err))
      logging.error( "Error: LookOut Cloud Security Anomaly data connector execution failed with an internal server error.")
      raise
