"""WAS Vuln DownloadChunk Orchestrator file."""
...
function_name = "TenableWASVulnDownloadChunkOrchestrator"
was_vuln_download_and_process_chunk_activity_name = "TenableWASVulnDownloadAndProcessChunks"


def check_table_exist():
    """Check if the WAS Vuln table exists in the storage account.
    ...
    for table in tables:
        if table.name == was_vuln_table_name:
            logging.info(f"{logs_starts_with} {function_name}: {was_vuln_table_name} table exists.")
            return True
    return False
...
def orchestrator_function(context: df.DurableOrchestrationContext):
    """
    Orchestrator function to handle Tenable.io WAS Vuln Export chunks.

    This function is responsible to check if the WAS Vuln table exists in the storage account.
    If the table exists, it will query for all queued chunks and call the activity function
    `TenableWASVulnDownloadAndProcessChunks` to download and process the chunks.
    ...
    Returns:
        None
    """
    logging.info(f"{logs_starts_with} {function_name}: Starting execution for TenableWASVulnDownloadChunkOrchestrator")
    logging.info(
    ...
    if not flag:
        logging.info(
            f"{logs_starts_with} {function_name}:{was_vuln_table_name} table is not created."
    ...
    while True:
        ...
        status, jobs_with_finished_chunks = yield context.call_activity(
            was_vuln_download_and_process_chunk_activity_name, str_activity_data
        )
        if not status:
