...
def check_table_exist():
    """Check if the compliance table exists in the storage account.
    ...
    for table in tables:
        if table.name == compliance_table_name:
            logging.info(
                f"{logs_starts_with} {function_name}: {compliance_table_name} table exists."
            )
            return True
    return False
...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    if not flag:
        logging.info(
            f"{logs_starts_with} {function_name}:{compliance_table_name} table is not created. waiting for next execution."
        )
    ...
    while True:
        ...
        logging.info(
            f"{logs_starts_with} {function_name}: Sorted chunks Data: {sorted_chunks}"
        )

        str_activity_data = json.dumps(sorted_chunks)
        status, jobs_with_finished_chunks = yield context.call_activity(
            compliance_download_and_process_chunk_activity_name, str_activity_data
        )
        if not status:
