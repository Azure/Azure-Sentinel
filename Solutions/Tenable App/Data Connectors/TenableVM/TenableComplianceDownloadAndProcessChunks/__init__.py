...
def main(sortedChunks: str):
    ...
    for chunk in chunk_ids_data:
        ...
        if update_checkpoint:
            logging.info(
                f"{logs_starts_with} {function_name}: Updating compliance checkpoint to value: {start_time}"
            )
        ...
        if chunk_data is None:
            logging.info(f"{logs_starts_with} {function_name}: Not able to download"
                         + f" data for the chunk : {chunk_id} Continuing to next chunk.")
            continue
