"""Orchestrator function for vuln export jobs."""
...
function_name = "TenableVulnExportOrchestrator"


def orchestrator_function(context: df.DurableOrchestrationContext):
    """
    Orchestrator function to handle Tenable.io Vuln Export Jobs.
    ...
    Returns:
        ...
        the vuln instance ID and the type of export
    """
    logging.info(f"{logs_starts_with} {function_name}: Started vuln export orchestrator")
    ...
    while True:
        logging.info(f"{logs_starts_with} {function_name}: Checking status of job {vuln_job_id}")
        str_job_status = yield context.call_activity(vuln_status_and_chunk, str_activity_data)
        try:
        ...
        logging.info(
            f"{logs_starts_with} {function_name}: {vuln_job_id} is currently in this state: {job_status['status']}"
        )
