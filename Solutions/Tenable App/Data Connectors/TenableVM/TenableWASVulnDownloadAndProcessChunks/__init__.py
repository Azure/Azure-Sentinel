...
def main(sortedChunks: str):
    ...
    for chunk in chunk_ids_data:
        ...
        if update_checkpoint:
            logging.info(f"{logs_starts_with} {function_name}: Updating WAS Vuln checkpoint to value: {start_time}")
        ...
        if chunk_data is None:
            ...
            was_vuln_table.update_if_found(job_id, str(chunk_id), {"jobStatus": TenableStatus.finished.value})
            continue
        update_count = send_chunk_details_to_sentinel(
            job_id, chunk_id, chunk_data, execution_start_time
        )
