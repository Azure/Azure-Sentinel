"""WAS Vuln DownloadChunk Activity file."""
...
log_type = os.environ.get("WASVulnTableName", "Tenable_WAS_Vuln")
scope = os.environ["SCOPE"]
logs_starts_with = "TenableVM"
function_name = "TenableWASVulnDownloadAndProcessChunks"
...
def download_chunk_details(job_id, chunk_id):
    ...
    and the error code indicates a retriable error (429 or 400), the function will retry
    after a brief delay.

    If the download is retried due to a retriable error, the function will log a message
    ...
    update the WAS Vuln table to indicate that the chunk has failed.
    ...
    """
    logging.info(f"{logs_starts_with} {function_name}: Using pyTenable client to download WAS Vuln export job chunk")
    logging.info(
        f"{logs_starts_with} {function_name}: Downloading chunk at was/v1/export/vulns/{job_id}/chunks/{chunk_id}"
    )
    tio = TenableIO()
    retries = 3
    status_code = None
    while retries > 0:
        try:
            chunk = tio.exports.download_chunk("was", job_id, chunk_id)
            logging.info(
                f"{logs_starts_with} {function_name}:Received a response from was/v1/export/vulns/{job_id}/chunks/{chunk_id}"
            )
            return True, chunk
            ...
            if status_code in [401, 403]:
                logging.info(
                    f"{logs_starts_with} {function_name}: Failed to retrive WAS Vuln chunk data. Error code {status_code}"
                )
                return False, None
                ...
                logging.warning(
                    f"{logs_starts_with} {function_name}: Failure to retrieve WAS Vuln data from Tenable."
                    f" Export Job ID: {job_id} Chunk ID: {chunk_id}. Error code {status_code}"
                )
                was_vuln_table.update_if_found(
                    job_id,
                    str(chunk_id),
                    {
...
def send_chunk_details_to_sentinel(job_id, chunk_id, chunk, execution_start_time):
    """
    Send a chunk of data from a Tenable WAS Vuln export job to Azure Sentinel.
    ...
    Args:
        ...
        chunk (list): The list of WAS Vuln data.
    ...
    try:
        ...
        was_vuln_table.update_if_found(job_id, str(chunk_id), {"jobStatus": TenableStatus.finished.value})
        return True
...
def update_checkpoint_for_last_chunk(update_checkpoint, start_time):
    """
    Update the checkpoint for the last chunk of WAS vuln data.
    ...
    if update_checkpoint:
        logging.info(f"{logs_starts_with} {function_name}: Updating WAS Vulns checkpoint to value: {start_time}")
        checkpoint_table = ExportsTableStore(connection_string, checkpoint_table_name)
        checkpoint_table.merge("was_vulns", "timestamp", {"was_vulns_timestamp": start_time})


def download_and_process_chunks(sorted_chunks, execution_start_time, jobs_with_finished_chunks):
    """
    Download and process chunks of WAS vuln data from Tenable.
...
def main(name: str):
    ...
    while True:
        ...
        queued_chunks = was_vuln_table.query_for_all_queued_chunks()
    ...
    logging.info(
        ...
        f" chunks available in {was_vuln_table_name} table."
    )
