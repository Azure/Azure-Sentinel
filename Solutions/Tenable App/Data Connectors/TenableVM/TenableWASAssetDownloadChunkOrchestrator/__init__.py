"""WAS Asset DownloadChunk Orchestrator file."""
...
def orchestrator_function(context: df.DurableOrchestrationContext):
    """
    Orchestrator function to handle Tenable.io WAS Asset Export chunks.

    This function is responsible to check if the WAS assets table exists in the storage account.
    If the table exists, it will query for all queued chunks and call the activity function
    `TenableWASAssetDownloadAndProcessChunks` to download and process the chunks.
    ...
    Returns:
        None
    """
    logging.info(f"{logs_starts_with} {function_name}: Starting execution for TenableWASAssetDownloadChunkOrchestrator")
    logging.info(
