...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    """
    logging.info(
        f"{logs_starts_with} {function_name}: Starting execution for TenablevulnDownloadChunkOrchestrator"
    )
    logging.info(
    ...
    status, jobs_with_finished_chunks = yield context.call_activity(
        vuln_download_and_process_chunk_activity_name
    )
    logging.info(f"{logs_starts_with} {function_name}: Activity status: {status}")
    logging.info(
        f"{logs_starts_with} {function_name}: Jobs with finished chunks: {jobs_with_finished_chunks}"
    )
    ...
    logging.info(
        f"{logs_starts_with} {function_name}: Number of finished chunks by job id: {jobs_with_finished_chunks}"
    )
    if not status:
        logging.error(
    ...
    logging.info(
        f"{logs_starts_with} {function_name}: Will check next at time: {next_check}"
    )
