"""Vuln DownloadChunk Activity file."""
...
log_type = os.getenv("VulnTableName", "Tenable_VM_Vuln")
...
def download_chunk_details(job_id, chunk_id):
    ...
    update the vuln table to indicate that the chunk has failed.
    ...
    """
    logging.info(f"{logs_starts_with} {function_name}: Using pyTenable client to download vuln export job chunk")
    logging.info(f"{logs_starts_with} {function_name}: Downloading chunk at vulns/export/{job_id}/chunks/{chunk_id}")
    tio = TenableIO()
    retries = 3
    status_code = None
    while retries > 0:
        try:
            chunk = tio.exports.download_chunk("vulns", job_id, chunk_id)
            logging.info(
                f"{logs_starts_with} {function_name}: Received a response from vulns/{job_id}/chunks/{chunk_id}"
            )
            return True, chunk
            ...
            if status_code in [401, 403]:
                logging.info(
                    f"{logs_starts_with} {function_name}: Failed to retrive vuln chunk data. Error code {status_code}"
                )
                return False, None
                ...
                logging.warning(
                    f"{logs_starts_with} {function_name}: Failure to retrieve vuln data from Tenable."
                    f" Export Job ID: {job_id} Chunk ID: {chunk_id}. Error code {status_code}"
                )
                vuln_table.update_if_found(
                    job_id,
                    str(chunk_id),
                    {
...
def send_chunk_details_to_sentinel(job_id, chunk_id, chunk, execution_start_time):
    """
    Send a chunk of data from a Tenable vuln export job to Azure Sentinel.
    ...
    Args:
        ...
        chunk (list): The list of vuln data.
    ...
    try:
        ...
        vuln_table.update_if_found(job_id, str(chunk_id), {"jobStatus": TenableStatus.finished.value})
        return True
    except Exception as e:
        vuln_table.update_if_found(job_id, str(chunk_id), {"jobStatus": TenableStatus.failed.value})
...
def update_checkpoint_for_last_chunk(update_checkpoint, start_time):
    """
    Update the checkpoint for the last chunk of Vuln data.
    ...
    if update_checkpoint:
        logging.info(f"{logs_starts_with} {function_name}: Updating Vulns checkpoint to value: {start_time}")
        checkpoint_table = ExportsTableStore(connection_string, checkpoint_table_name)
        checkpoint_table.merge("vulns", "timestamp", {"vulns_timestamp": start_time})


def download_and_process_chunks(sorted_chunks, execution_start_time, jobs_with_finished_chunks):
    """
    Download and process chunks of Vuln data from Tenable.
...
def main(name: str):
    ...
    logging.info(
        ...
        f" chunks available in {vuln_table_name} table."
    )
