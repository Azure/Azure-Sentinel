...
class Alert:
    ...
    def __init__(self, json_str):
        ...
        if self.cluster_incarnation_id is None:
            raise ValueError("clusterIncarnationId is not present in the propertyList")
        self.protection_group_id = f"{self.cluster_id}:{self.cluster_incarnation_id}:{self.job_id}"
