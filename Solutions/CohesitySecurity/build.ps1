...
$buildFile = Join-Path -Path $PSScriptRoot -ChildPath "Package" | Join-Path -ChildPath "mainTemplate.json"
((Get-Content -path $buildFile -Raw) -replace 'send_to_email_for_playbook', $send_to_email_for_playbook) | Set-Content -Path $buildFile
((Get-Content -path $buildFile -Raw) -replace 'cohesity_support_email', $cohesity_support_email) | Set-Content -Path $buildFile
((Get-Content -path $buildFile -Raw) -replace 'apiKey_string', $apiKey_string) | Set-Content -Path $buildFile
