...
class StateManager:
    def __init__(
        ...
        file_path="Eventsfuncstatemarkerfile",
    ):
...
def main(mytimer: func.TimerRequest) -> None:
    ...
    try:
        ...
        searchId = Lookout.get_SearchId(
            "/apigw/v1/events/searchId", "Activity", startTime, endTime
        )
        if searchId != None:
            results_events = Lookout.get_Data("/apigw/v1/events", searchId)
            logging.info(
                "The number of Activities processed {} ".format(len(results_events))
            )
            ...
            logging.info("End: failed to get Activity (searchId)")
