"""Get mimecast ci data and ingest to custom table in sentinel."""
...
class MimecastCIToSentinel(Utils):
    """Class for ingest the data from mimecast to sentinel."""

    def __init__(self, start_time) -> None:
        """Initialize MimecastCIToSentinel object."""
        super().__init__(consts.CLOUD_INTEGRATED_FUNCTION_NAME)
    ...
    async def get_mimecast_ci_data_in_sentinel(self):
        """Get mimecast ci data and ingest data to sentinel, initialization method."""
        ...
        try:
            applogger.info(
                self.log_format.format(
                    ...
                    "Start fetching ci endpoint data using batch and async",
                )
            )
    ...
    async def get_batch_data_urls_from_api(self):
        """Retrieve a list of URLs from the Mimecast CI API and processes them.

        This function retrieve a list of URLs from the Mimecast CI API by making a GET request to the
        CI endpoint. It iterate through the response pages and retrieves the URLs from each page.
        ...
        try:
            ...
            if checkpoint_data:
                ...
                checkpoint_data = {}
            url = "{}{}".format(consts.BASE_URL, consts.ENDPOINTS["CI"])

            params = {"type": consts.CI_TYPES, "pageSize": consts.ASYNC_PAGE_SIZE}
            page = 1
            while True:
