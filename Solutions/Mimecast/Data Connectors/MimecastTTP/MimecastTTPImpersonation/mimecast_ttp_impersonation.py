"""Get Mimecast TTP Impersonation Data and Ingest into Sentinel."""
...
file_path = "mimecastttpimpersonation"


class MimecastTTPImpersonation(Utils):
    """Mimecast TTP Impersonation Class."""
    ...
    def __init__(self, start_time) -> None:
        ...
        """
        super().__init__(consts.TTP_IMPERSONATION_FUNCTION_NAME)
    ...
    def get_mimecast_ttp_impersonation_data_in_sentinel(self):
        """Get the TTP Impersonation Data from Mimecast."""
    ...
    def get_and_ingest_data_to_sentinel(self, from_date, to_date, page_token):
        ...
        try:
            ...
            while True:
                ...
                url = "{}{}".format(
                    consts.BASE_URL, consts.ENDPOINTS["TTP_IMPERSONATION"]
                )
                ...
                data_to_ingest = response.get("data")[0].get("impersonationLogs")
                applogger.info(
                    self.log_format.format(
                        consts.LOGS_STARTS_WITH,
                        __method_name,
