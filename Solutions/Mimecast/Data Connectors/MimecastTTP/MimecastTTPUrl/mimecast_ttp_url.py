"""Get Mimecast TTP URL Data and Ingest into Sentinel."""
...
class MimecastTTPUrl(Utils):
    """Mimecast TTP Url Class."""

    def __init__(self, start_time) -> None:
        """Initialize the MimecastTTPUrl class.

        Args:
            start(int): The starting time for the timer trigger.
        """
        super().__init__(consts.TTP_URL_FUNCTION_NAME)
    ...
    def get_mimecast_ttp_url_data_in_sentinel(self):
        """Get the TTP Url Data from Mimecast."""
    ...
    def get_and_ingest_data_to_sentinel(self, from_date, to_date, page_token):
        ...
        try:
            ...
            while True:
                ...
                url = "{}{}".format(consts.BASE_URL, consts.ENDPOINTS["TTP_URL"])
                ...
                data_to_ingest = response.get("data")[0].get("clickLogs")
                applogger.info(
                    self.log_format.format(
                        consts.LOGS_STARTS_WITH,
                        __method_name,
                ...
                if len(data_to_ingest) > 0:
                    send_data_to_sentinel(data_to_ingest, consts.TABLE_NAME["TTP_URL"])
