...
LEVEL_OVERALL_CHANGE = os.environ["LEVEL_RATINGS_CHANGE"]
PORTFOLIO_IDS_STR = os.environ["PORTFOLIO_IDS_STR"]
DIFF_OVERRIDE_OWN_OVERALL = os.environ["DIFF_OVERRIDE_OWN_RATINGS"]
DIFF_OVERRIDE_PORTFOLIO_OVERALL = os.environ["DIFF_OVERRIDE_PORTFOLIO_RATINGS"]
...
def main(mytimer: func.TimerRequest) -> None:
    ...
    try:
        ...
        domain = DOMAIN.strip()
        level_overall_change = LEVEL_OVERALL_CHANGE.strip()
        portfolio_ids_str = PORTFOLIO_IDS_STR.strip()
        diff_override_own_overall = str(DIFF_OVERRIDE_OWN_OVERALL).strip().lower()
        diff_override_portfolio_overall = str(DIFF_OVERRIDE_PORTFOLIO_OVERALL).strip().lower()
        ...
        if check_date != str(to_date):
            # Fetching data of domain OverallScore and writing it into the Microsoft Sentinel
            scorecard_obj = ScoreCardHelperClass(base_url, secret_key, domain)
            writer_obj = CompanyWriter(scorecard_obj, state_manager_object)
            logging.info(
                "SecurityScorecard Connector: Writting Overall data into the Microsoft Sentinel!"
            )
            writer_obj.write_overall(**config)
            logging.info(
                "SecurityScorecard Connector: Overall data successfully ingested!"
            )
        else:
        ...
        for from_date_company in company_to_fetch_data.keys():
            ...
            for company_data in date_wise_company_list:
                ...
                writer_obj.write_overall(company_pid, **config)
                logging.info(
                    "SecurityScorecard Connector: Overall data successfully ingested!"
                )
        ...
        raise SSOverallScoreException("SecurityScorecard Connector: {}".format(err))
