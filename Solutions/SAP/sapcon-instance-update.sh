...
grepvars="ABAP\|JAVA\|LOG_WS\|AZURE_"
...
do
        ...
        then
                ...
                # getting env vars to env.file.current
                rm ./env.file.current > /dev/null 2>&1
                docker inspect -f "{{ .Config.Env }}" $contname | tail -c +2 | head -c -2| sed 's/\s\+/\n/g' | grep $grepvars> env.file.current
                if [  ! -z $contid ]
                then
                        ...
                        then
                                ...
                                then
                                                ...
                                                docker create -v --env-file=./env.file.current $sysfileloc:/sapcon-app/sapcon/config/system --name $contname $dockerimage$tagver >/dev/null
						#removing env file
                                                rm ./env.file.current > /dev/null 2>&1
                                        ...
                                        docker container rm $contname  >/dev/null
                                        docker create --env-file=./env.file.current -v $sysfileloc:/sapcon-app/sapcon/config/system --name $contname $dockerimage$tagver  >/dev/null
                                        #removing env file
					rm ./env.file.current > /dev/null 2>&1
