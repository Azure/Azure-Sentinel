...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    # Get ResourceType, ResourceId and TagKeys from the request parameters
    resource_type = req.params.get('ResourceType')
    resource_id = req.params.get('ResourceId')
    tag_keys = req.params.get('TagKeys')

    if not (resource_type and resource_id and tag_keys):
        try:
            ...
            tag_keys = req_body.get('TagKeys')

    if resource_type and resource_id and tag_keys:
        try:
            ...
            try:
                logging.info('Calling function to remove tags to resources.')

                results = ssm_client.remove_tags_from_resource(
                    ResourceType=str(resource_type),
                    ResourceId=str(resource_id),
                    TagKeys=list(tag_keys)
                )

                logging.info('Call to remove tags to resources successful.')
        ...
        return func.HttpResponse(
             "Pass ResourceType, ResourceId and TagKeys in the query string or request body.",
             status_code=400
        )
