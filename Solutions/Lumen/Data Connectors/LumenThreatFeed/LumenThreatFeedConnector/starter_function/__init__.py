...
def download_threat_data_to_shared_disk(updater, indicator_types, max_indicators=None):
    ...
    # Optional: allow skipping the post-download full scan/count (saves time on large files)
    ...
    for indicator_type, presigned_url in presigned_urls.items():
        ...
        try:
            ...
            filtered_count = 0
            if enable_counting:
                # Single pass count with confidence threshold (still a second pass over file, but now optional)
                try:
                    ...
                    logging.warning(f"Counting failed for {indicator_type}: {ex}")
                    indicator_count = 0
                    filtered_count = 0
            else:
                # Skip counting entirely – orchestrator will treat as unknown size (-1)
                indicator_count = -1
            ...
            if enable_counting:
                if filtered_count > 0:
                    logging.info(f"Downloaded {indicator_type}: {downloaded_bytes} bytes, ~{indicator_count + filtered_count} total indicators, ~{indicator_count} after confidence filtering (filtered out {filtered_count}) to {temp_file_path}")
                else:
                    logging.debug(f"Downloaded {indicator_type}: {downloaded_bytes} bytes, ~{indicator_count} indicators to {temp_file_path}")
            else:
                logging.info(f"Downloaded {indicator_type}: {downloaded_bytes} bytes (indicator count skipped – set ENABLE_INDICATOR_COUNT=1 to enable)")
    ...
    if enable_counting:
        logging.info(f"Download complete. Total files: {len(temp_file_paths)}, Total indicators (after confidence filtering): ~{total_indicators_downloaded}")
    else:
        logging.info(f"Download complete. Total files: {len(temp_file_paths)} (counts skipped – set ENABLE_INDICATOR_COUNT=1 to enable)")
...
async def main(req: func.HttpRequest, starter: str) -> func.HttpResponse:
    ...
    try:
        ...
        logging.info(f"   Total indicators queued for upload (after confidence filtering): ~{total_indicators}")
