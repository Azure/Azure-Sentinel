...
def download_threat_data_to_shared_disk(updater, indicator_types, max_indicators=None):
    ...
    for indicator_type, presigned_url in presigned_urls.items():
        ...
        try:
            ...
            with open(temp_file_path, 'rb') as f:
                indicator_count = 0
                try:
                    for _ in ijson.items(f, 'stixobjects.item'):
                        indicator_count += 1
    ...
    logging.info(f"Download complete. Total files: {len(temp_file_paths)}, Total indicators: ~{total_indicators_downloaded}")
...
async def main(req: func.HttpRequest, starter: str) -> func.HttpResponse:
    ...
    try:
        ...
        logging.debug("Downloading threat data to shared disk for orchestrator processing...")
        ...
        logging.debug(f"Download complete. Files: {len(temp_file_paths)}, Total indicators: ~{total_indicators}")
        ...
        logging.debug(f"🎯 Operation Started Successfully:")
        logging.debug(f"   Total indicators queued for upload: ~{total_indicators}")
        logging.debug(f"   Elapsed time for initialization: {elapsed_time:.2f} seconds ({elapsed_minutes:.2f} minutes)")
        logging.debug(f"   Orchestration ID: {instance_id}")
        logging.debug(f"   Status: Threat intelligence upload orchestration initiated")
