...
class LumenSentinelUpdater:
    ...
    def stream_and_filter_to_blob(self, container_client, indicator_type: str, presigned_url: str, run_id: str) -> Dict[str, Any]:
        """Stream threat data from Lumen and filter to blob storage (V5 Architecture)."""
        ...
        try:
            # Download data
            response = self.limiter_session.get(presigned_url, timeout=300)
            response.raise_for_status()
            ...
            logging.info(f"   • Confidence ≥{confidence_threshold}: {filtered_count:,} objects ({filter_rate:.1f}%)")
