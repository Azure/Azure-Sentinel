...
This playbook extracts tags from incident comments and adds them to each detection listed in the incident's custom details. If no comment with proper structure is found and Teams is configured, it prompts the user for tag input via an Adaptive Card. If Teams is not configured, the playbook exits gracefully with Succeeded status.
...
3. *(Optional)* Obtain Teams GroupId and ChannelId if you want the playbook to prompt for tags when no structured comment is found.
...
4. Ensure the VectraGenerateAccessToken playbook is deployed before deploying VectraAddTagToDetections playbook.
...
1. In Microsoft Sentinel Incident, the comment should be in the following structure only to be able to extract values from it.
   ...
   * **tag: {tag_value}** (Do not use quotes for tag value)
   * Single tag value is supported from the comment. For multiple tag values, use the adaptive card option instead.
...
3. Tags are merged with any existing tags on each detection — existing tags are never removed.
