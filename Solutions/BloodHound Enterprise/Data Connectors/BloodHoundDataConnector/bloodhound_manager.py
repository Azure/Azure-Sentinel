...
import hashlib
import base64
from urllib.parse import urljoin, urlparse, urlencode, parse_qs  # Import parse_qs
import datetime
import json
import re  # Import regex module for dynamic format_kwargs detection
...
class BloodhoundManager:
    ...
    def _get_headers(self, method: str, uri: str, payload: str = None) -> dict:
        ...
        try:
            ...
            print("fromated date", datetime_formatted)  # Debug print to check datetime format
    ...
    def get_audit_logs(self) -> dict | None:
        """
        ...
        while True:
            ...
            uri: str = f"/api/v2/audit?skip={skip}&limit={limit}"
    ...
    def get_finding_trends(self, environment_id) -> dict | None:
        """
        ...
        """
        params = {}
        uri: str = f"/api/v2/attack-paths/finding-trends?environments={environment_id}"
        self.logger.info(f"Fetching finding trends with params: {params}")
        return self._api_request(uri)

    def get_posture_history(self, data_type: str, environment_id=None) -> dict | None:
        """
        ...
        uri: str = f"/api/v2/posture-history/{data_type}?environments={environment_id}"
        return self._api_request(uri)
    ...
    def get_attack_path_sparkline_timeline(
        self, domain_id: str, finding_type: str
    ) -> list:
        """
        ...
        try:
            page_data = self._api_request(
                f"/api/v2/domains/{domain_id}/sparkline?finding={finding_type}"
            )
    ...
    def send_audit_logs_data(self, data: dict, bearer_token: str) -> dict:
        ...
        api_url = f"{self.dce_uri}/dataCollectionRules/{self.dcr_immutable_id}/streams/Custom-{self.table_name}?api-version=2023-01-01"
        headers = {
        ...
        try:
            response = requests.post(
                api_url,
                headers=headers,
                data=json.dumps([log_entry]),  # DCE expects an array of logs
            )
            response.raise_for_status()
            response_content = (
                response.json()
            ...
            self.logger.info(
                f"Audit log entry ID {log_entry.get('id')} sent successfully to Azure Monitor. Response: {response_content}"
            )
            return {"status": "success", "response": response_content}
        except requests.RequestException as e:
            self._log_error(
                f"Error sending audit log entry ID {log_entry.get('id')} to Azure Monitor: {e} - Response: {getattr(e.response, 'text', 'N/A')}"
            )
    ...
    def send_finding_trends_logs(
        ...
        try:
            ...
            self.logger.info(
                f"Finding trends log for '{log_entry.get('finding')}' sent successfully. Response: {response_content}"
            )
            return {"status": "success", "response": response_content}
        except requests.RequestException as e:
            self._log_error(
                f"Error sending finding trends log for '{log_entry.get('finding')}' to Azure Monitor: {e} - Response: {getattr(e.response, 'text', 'N/A')}"
            )
            return {"status": "error", "message": str(e)}
    ...
    def send_posture_history_logs(
        ...
        try:
            ...
            self.logger.info(
                f"Posture history log for type '{log_entry.get('data_type')}' sent successfully. Response: {response_content}"
            )
            return {"status": "success", "response": response_content}
        except requests.RequestException as e:
            self._log_error(
                f"Error sending posture history log for type '{log_entry.get('data_type')}' to Azure Monitor: {e} - Response: {getattr(e.response, 'text', 'N/A')}"
            )
            return {"status": "error", "message": str(e)}
    ...
    def send_attack_data(
        ...
        try:
            ...
            self.logger.info(
                f"Attack path ID {log_entry.get('id')} sent successfully to Azure Monitor. Response: {response_content}"
            )
            return {"status": "success", "response": response_content}
        except requests.RequestException as e:
            self._log_error(
                f"Error sending attack path ID {log_entry.get('id')} to Azure Monitor: {e} - Response: {getattr(e.response, 'text', 'N/A')}"
            )
            return {"status": "error", "message": str(e)}
    ...
    def send_attack_path_timeline_data(
        ...
        try:
            ...
            response.raise_for_status()
            self.logger.info(
                f"Successfully sent attack data ID {attack_data.get('id')} to Azure Monitor."
            )
            return response.json() if response.content else {"status": "success"}
        except requests.RequestException as e:
            self._log_error(
                f"[Send Error] Failed to send attack data ID {attack_data.get('id')} to Azure Monitor: {e}. Response: {e.response.text if e.response else 'N/A'}"
            )
            return {"status": "error", "message": str(e)}
    ...
    def send_tier_zero_assets_data(
        ...
        try:
            ...
            response.raise_for_status()
            self.logger.info(
                f"Successfully sent Tier Zero asset ID {node_id} to Azure Monitor."
            )
            return response.json() if response.content else {"status": "success"}
        except requests.RequestException as e:
            self._log_error(
                f"[Send Error] Failed to send Tier Zero asset ID {node_id} to Azure Monitor: {e}. Response: {e.response.text if e.response else 'N/A'}"
            )
            return {"status": "error", "message": str(e)}
