...
from integration.models import Config, DataSource, TokenStorage
...
class ServiceClientSentinel(ServiceClient):
    ...
    def _get_last_data_time_handler(self, data_source: DataSource) -> LastDataTimeHandlerSentinel:
    ...
    def _validate_if_run_instance_old_version(self, data_source: DataSource) -> bool:
        if data_source == DataSource.EI_ECOS and all(
    ...
    async def _old_version_check(
        self, data_source: DataSource, last_data_time_handler: LastDataTimeHandlerSentinel
    ) -> tuple[DataSource, LastDataTimeHandlerSentinel]:
        if (
            self.config.version == "3.0.0"
            and data_source == DataSource.EI_ECOS
            ...
            if not response_data or not response_data.get("nextDeltaToken"):
                data_source, last_data_time_handler = (
                    DataSource.EP,
                    LastDataTimeHandlerSentinel(DataSource.EP, self.env_vars.interval, self.env_vars.conn_str),  # type: ignore
                )
