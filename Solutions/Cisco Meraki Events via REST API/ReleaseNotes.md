...
|-------------|--------------------------------|--------------------------------------------------------------------|
| 3.1.1       | 10-08-2026                     | Updated CCP Connector to use checkpointed `t0`/`t1` time windowing (UnixTimestamp) for the Air Marshal Events and Network Clients data types instead of a fixed timespan, aligning them with the existing ASIM log endpoints to prevent duplicate ingestion. |
