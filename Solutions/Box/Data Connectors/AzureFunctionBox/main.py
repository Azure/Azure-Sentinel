...
def main(mytimer: func.TimerRequest):
    ...
    created_after = parse_datetime(
        str(created_after)) + datetime.timedelta(seconds=1)
    ...
    if parse_datetime(str(created_after)) + datetime.timedelta(minutes=MAX_PERIOD_MINUTES) < parse_datetime(str(created_before)):
        created_before = parse_datetime(
            str(created_after)) + datetime.timedelta(minutes=MAX_PERIOD_MINUTES)
        logging.info('Backlog to process is more than than {} minutes. So changing created_before to {}. Remaining data will be processed during next invocation'.format(
            MAX_PERIOD_MINUTES, created_before))
    ...
    date_time_str_before = '2024-05-28 5:49:00+00:00'
    date_time_str_after = '2024-05-28 4:49:00+00:00'
...
def get_stream_pos_and_date_from(marker, max_period_minutes, script_execution_interval_minutes):
    ...
    def get_default_date_from(script_execution_interval_minutes):
        ...
        ) - datetime.timedelta(minutes=Historical_Data_Days*24*60)
    ...
    def get_token_from_marker(marker, max_period_minutes):
        ...
        try:
            ...
            minutes_from_last_ingested_event = (datetime.datetime.utcnow().replace(
                tzinfo=datetime.timezone.utc) - last_event_date).total_seconds() / 60
...
def save_marker(state_manager, stream_position, last_event_date):
    logging.getLogger().setLevel(logging.INFO)
    logging.info('Saving last stream_position {} and last_event_date {}'.format(
        stream_position, last_event_date))
...
def get_events(start_time, config_dict, created_after=None, created_before=None, stream_position=0):
    ...
    while True:
        if parse_datetime(str(created_after)) >= parse_datetime(str(created_before)):
            logging.warning(
                f"Created After param {created_after} is greater then Created Before param {created_before} ")
            break
        stream_position = 0
        res = events_client.get_events(start_time, stream_position=stream_position,
        ...
        if len(events) < limit:
            break
        created_after = get_last_event_ts(
            events=events, last_ts=created_after, field_name='created_at') + datetime.timedelta(seconds=1)
