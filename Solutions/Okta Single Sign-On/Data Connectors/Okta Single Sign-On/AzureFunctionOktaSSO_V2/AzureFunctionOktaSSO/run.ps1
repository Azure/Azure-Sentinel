...
if($null -eq $row.uri){
    $uri = "$uri$($StartDate)&limit=1000"
    Write-Host "OKTASSO uri: $uri !"
    Write-Host "Row $row"
    Write-Host "table $Table -PartitionKey 'part1' -RowKey $apiToken -property @{'uri'=$uri} -UpdateExisting"
    try {
        $result = Add-AzTableRow -table $Table -PartitionKey "part1" -RowKey $apiToken -property @{"uri"=$uri} -UpdateExisting
    }
    catch {
        Write-Host 'Error while updating Azure Table'
