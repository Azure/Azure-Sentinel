...
|-------------|--------------------------------|--------------------|
| 3.0.8 | 10-09-2026 | Removed the `fromTime`/`toTime` query parameters and time window filtering from the CCF poller; the `/Client/Anomaly` API is now called without a time filter on every poll. |
| 3.0.7 | 27-08-2026 | Fixed the CCF poller's Unix timestamp formatting for the `fromTime` and `toTime` query parameters. |
