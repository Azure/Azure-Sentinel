...
try:
    ...
    get_identity_servers_url = f"{EnvironmentEndpointURL}/IdentityServers"
    ...
    saml_identity_servers = [server for server in res.get("identityServers",[]) if server.get("samlType",0) == 1]

    print(f"Count of SAML Identity Servers found : {len(saml_identity_servers)}")
    ...
    for saml_identity_server in saml_identity_servers:
        ...
        getsaml_identity_server_prop_url = f"{EnvironmentEndpointURL}/V4/SAML/{saml_identity_server_name}"
        ...
        if getsaml_identity_server_prop_result.status_code == 200:
            ...
            if saml_server_details.get("enabled", False):
                ...
                disablesaml_identity_server_url = f"{EnvironmentEndpointURL}/V4/SAML/{saml_identity_server_name}"
