...
def main(mytimer: func.TimerRequest, checkpoint: str) -> str:
    ...
    try:
        customer_feeds = threat_intel_feed_request_helper.get_threat_intel_feed(mimecast_start_date, mimecast_end_date,
                                                                                'malware_customer')
    except MimecastRequestError as e:
        logging.error('Failed to get TI logs from Mimecast.',
                      extra={'request_id': threat_intel_feed_request_helper.request_id})
    ...
    try:
        if customer_feeds:
            latest_feed = threat_intel_feed_request_helper.send_feeds_to_azure(customer_feeds)
            return DateHelper.convert_to_mimecast_format(latest_feed['fileCompileDateTime'], "%Y-%m-%dT%H:%M:%SZ")
        else:
            logging.info("There are no Targeted Threat Intel Feeds for this period.")
