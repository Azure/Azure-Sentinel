...
class ThreatIntelFeedRequestHelper:
    ...
    def get_threat_intel_feed(self, start_date, end_date, feed_type):
        ...
        self.set_request_credentials(email=os.environ['mimecast_email'],
        ...
        model = GetThreatIntelFeedRequest(start_date, end_date, feed_type)
        ...
        success_response = response_helper.parser_threat_intel_feed_success_response(response,
                                                                                     'csv',
                                                                                     True,
                                                                                     MimecastEndpoints.get_threat_intel_feed)
    ...
    def find_latest_feed(feeds):
        latest_feed = max(feeds, key=lambda x: datetime.datetime.strptime(x['fileCompileDateTime'], "%Y-%m-%dT%H:%M:%SZ"))
        return latest_feed
    ...
    def send_feeds_to_azure(self, feeds):
        ...
        latest_feed = {}
        unique_mimecast_feeds = self.filter_out_duplicates(feeds)
        chunks_of_feeds = self.get_chunks(unique_mimecast_feeds)
        for chunk in chunks_of_feeds:
            ...
            # TODO: add duplicate check (waiting for feedback from microsoft support)
            # this code should replace the line above
            # feeds_to_update, feeds_to_create = graph_api_collector.find_already_submitted(headers, mapped_feeds)
            # graph_api_collector.update_threat_indicators(headers, feeds_to_update)
            # graph_api_collector.create_threat_indicators(headers, feeds_to_create)

            current_latest_feed = self.find_latest_feed(mapped_feeds)
            if latest_feed:
                if latest_feed.get('fileCompileDateTime') < current_latest_feed.get('fileCompileDateTime'):
                    latest_feed = current_latest_feed
            else:
                latest_feed = current_latest_feed
        return latest_feed
