...
def _wrap_core(
    ...
    for j in range(6):
        for i in range(n):
            ...
            b = encryptor.update(a + r[i])  # CodeQL [SM02597] Being used for backwards compatibility
            a = (
...
def _unwrap_core(
    ...
    for j in reversed(range(6)):
        for i in reversed(range(n)):
            ...
            b = decryptor.update(atr)   # CodeQL [SM02597] This is for backward compatibility. Not being used in functions app.
            a = b[:8]
            r[i] = b[-8:]
...
def aes_key_wrap_with_padding(
    ...
    if len(key_to_wrap) == 8:
        ...
        encryptor = Cipher(AES(wrapping_key), ECB()).encryptor()
        b = encryptor.update(aiv + key_to_wrap)  # CodeQL [SM02597] This is for backward compatibility. Not being used in functions app.
        assert encryptor.finalize() == b""
        return b
    else:
        r = [key_to_wrap[i : i + 8] for i in range(0, len(key_to_wrap), 8)]
...
def aes_key_unwrap_with_padding(
    ...
    if len(wrapped_key) == 16:
        ...
        decryptor = Cipher(AES(wrapping_key), ECB()).decryptor()
        out = decryptor.update(wrapped_key)     # CodeQL [SM02597] This is for backward compatibility. Not being used in functions app.
        assert decryptor.finalize() == b""
        a = out[:8]
        data = out[8:]
        n = 1
