...
|-------------|--------------------------------|---------------------------------------------------------------------------------------|
| 3.0.16      | 05-08-2026                     | Fixed column ordering bug in **Analytic Rule** AnomalousVoulmeOfFileDeletion.yaml - moved `UserId` to be the first projected column in `riskyUsers` so that the `in` operator correctly compares `AccountObjectId` against `UserId` instead of `RiskLevelDuringSignIn`. |
