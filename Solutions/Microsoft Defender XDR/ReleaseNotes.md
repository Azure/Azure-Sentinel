...
|-------------|--------------------------------|---------------------------------------------------------------------------------------|
| 3.0.16       | 19-06-2026                     | Updated `Microsoft Defender XDR` to ProductName filter in **Data Connector** queries. Added new **Hunting Queries** Hunt for RMM tool execution following Teams messages, Hunt for alerts correlated with Teams messages and Identify acting user for reported phish. Also updated **Hunting Query** Punycode chars lookalike and corrected the incorrect mapping of `dataTypes: EmailEvents` with `connectorId: OfficeATP` across multiple **Hunting Queries**. |
