...
def main(myTimer: func.TimerRequest) -> None:
    ...
    logging.info("Ipinfo CORE timer trigger function executed.")

    def upload_data_to_CORE_table(dce_endpoint, dcr_immutableid, stream_name):
        credential = ClientSecretCredential(TENANT_ID, CLIENT_ID, CLIENT_SECRET)
        client = LogsIngestionClient(endpoint=dce_endpoint, credential=credential, logging_enable=True)
        mmdb_file_path = "/tmp/ipinfo_core.mmdb"
        reader = maxminddb.open_database(mmdb_file_path)
        chunk_size = 10000
        data_chunk = []
        logging.info("Uploading CORE Data.\n")
        ...
        if data_chunk:
            try:
                ...
                logging.error(f"Upload failed: {e}")
        reader.close()
        logging.info("CORE Data uploading completed.")
    ...
    check_and_create_table(CORE_TABLE_NAME, CORE_TABLE_SCHEMA, access_token)
    retries = 3
    while retries > 0:
        if get_table(CORE_TABLE_NAME, access_token):
            logging.info("Waiting for the table to be created properly, creating the data collection rule in 1 minute...")
            time.sleep(60)
            CORE_dcr_immutableid, CORE_stream_name = check_and_create_data_collection_rules(
                access_token,
                CORE_DCR_NAME,
                CORE_STREAM_DECLARATION,
                CORE_TABLE_COLUMNS,
                DATA_COLLECTION_ENDPOINT_NAME,
            )
            upload_data_to_CORE_table(dce_endpoint, CORE_dcr_immutableid, CORE_stream_name)
            break
        else:
