...
def parse_indicator_type_filter(config):
    ...
    split_indicators = [ind.strip() for ind in config.indicators.split(",") if ind.strip()]
...
def convert_to_stix(indicators, config):
    ...
    for indicator in indicators:
        stix_object = {
            ...
            "spec_version": "2.1",
            "type": "indicator",
            "created": datetime.fromtimestamp(indicator["published_date"], tz=timezone.utc).isoformat(),
            "modified": datetime.fromtimestamp(indicator["last_updated"], tz=timezone.utc).isoformat(),
            "name": indicator["indicator"],
            "description": "",
            "indicator_types": indicator["threat_types"],
            "pattern_type": "stix",
            "valid_from": datetime.fromtimestamp(
                indicator["published_date"], tz=timezone.utc
            ).isoformat(),
            "kill_chain_phases": [
...
def run(mytimer: func.TimerRequest) -> None:
    """
    Azure Function entry point that fetches CrowdStrike indicators
