#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# build.sh — Build and validation for Tanium Azure Sentinel solution.
# Source this file from Solutions/Tanium/ci (do not run as a standalone script).
# Entry points:
#   build_solution
#   check_prerequisites
#   check_spelling
...
#  RESPONSE FUNCTIONS
###########################################
# Printed with default color (with trailing new line).
_msg() {
	echo >&2 -e "${1-}"
}

# Printed with default color (no trailing new line).
_msg_print() {
	echo >&2 -n -e "${1-}"
}
...
# Printed in cyan.
_msg_info() {
	echo >&2 -e "\033[0;36m$1\033[0m"
}

# Prefixes with success icon (default color).
_msg_success() {
	_msg "  ✅ $1"
}

# Printed in bold (default color).
_shout() {
	_msg
...
validate_manifest() {
	...
	status=$?
	manifest_failed=$(( manifest_failed || status ))
	...
	if [[ $manifest_failed -ne 0 ]]; then
		_die "Manifest validation failed"
	fi
}
...
_validate_solution_resources() {
	...
	if [[ -n "$undeclared_files" ]]; then
		...
		while IFS= read -r line; do
			[[ -n "$line" ]] && _msg "        - $line"
		done <<< "$undeclared_files"
		_msg
		_msg_info "Did you forget to add them to Solutions/Tanium/Data/Solution_Tanium.json?"
		_msg
		return 1
	fi
	...
	if [[ -n "$missing_files" ]]; then
		...
		_msg
		_msg_info "Did you forget to add them to Solutions/Tanium/${expected_folder}?"
		_msg
		return 1
	fi
}
...
check_spelling() {
	...
	if ! cspell --quiet . --exclude ./Workbooks/connect-module-connections.json --exclude ./Package/mainTemplate.json --reporter @cspell/cspell-json-reporter >"$error_file"; then
		...
		_msg_error "Found ${error_count} misspellings! Please check the ci folder for the list."
		return 1
	else
...
# Optional first argument: VersionBump (major, minor, or patch).
# When provided, uses local version mode for the build and get-new-version; when omitted, uses catalog mode.
build_solution() {
	local version_bump=$1

	local ci_directory
	ci_directory="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"

	local data_directory
	data_directory="${ci_directory}/../Data"

	# Solution version must be obtained BEFORE the build because in local mode the build increments the manifest version.
	local new_version
	if [[ -n "$version_bump" ]]; then
		new_version=$(pwsh ./get-new-version.ps1 "$data_directory" -VersionBump "$version_bump") || _die "get-new-version.ps1 failed"
	else
		new_version=$(pwsh ./get-new-version.ps1 "$data_directory") || _die "get-new-version.ps1 failed"
	fi
	if [[ -z "$new_version" ]]; then
		_die "get-new-version.ps1 did not return a version. Your solution is not configured correctly."
	fi
...
deploy_to_azure() {
	...
	local zip_path
	zip_path="${_package_folder_path}/${version}.zip"
	if [[ ! -f "$zip_path" ]]; then
	...
	if ! unzip -j -o "$zip_path" mainTemplate.json -d "$temporary_directory" >/dev/null 2>&1; then
		_die "Failed to extract mainTemplate.json from $zip_path. Is the zip valid?"
	fi
	if [[ ! -f "${temporary_directory}/mainTemplate.json" ]]; then
	...
	if (! az deployment group create \
		...
		--template-file "${temporary_directory}/mainTemplate.json" \
	...
	_msg_warning "\n⚠️  Please verify all the template data in the workspace ${workspace} and complete any final QA tasks."
}
