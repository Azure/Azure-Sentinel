...
OneLoginBearerToken = os.environ['OneLoginBearerToken']
...
def post_data_to_sentinel(body):
    ...
    if (response.status_code >= 200 and response.status_code <= 299):
        logging.info(
            "TheHive event successfully processed to the Azure Sentinel.")
        return response.status_code
    else:
        logging.warn("Event is not processed into Azure. Response code: {}".format(
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    logging.info(
        'Python HTTP trigger function processed a request. Start of processing.')
    method = req.method
    params = req.params
    if method == 'POST':
        if 'Authorization' in req.headers:
            bearer_string = re.match(
                "^Bearer\s+(.*)", req.headers['Authorization'])
            if bearer_string:
                bearer_token = bearer_string.group(1)
                if bearer_token == OneLoginBearerToken:
                    post_req_data = req.get_body()
                    logging.info(
                        f"Post request data: {post_req_data}")
                    message = post_req_data.decode('utf-8')
                    logging.info(
                        f"Getting messages: {message}")
                    ...
                    try:
                        ...
                        logging.error(
                            f"Is not valid Message format. Error: {err}. Message: {message}.")
                    return func.HttpResponse("200 OK HTTPS", status_code=200)
                else:
                    logging.error(
                        "Unauthorized, Bearer token is not right. Error code: 401.")
                    return func.HttpResponse("Unauthorized, Bearer token is not right.", status_code=401)
            else:
                logging.error(
                    "Unauthorized, Bearer token is not present in the request. Error code: 401.")
