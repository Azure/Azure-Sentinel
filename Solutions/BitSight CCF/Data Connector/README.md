...
1. `bitsight-risk-findings-connection-template.json` - Connection template that creates the custom table, per-company DCE, per-company DCR, and ten `RestApiPoller` connections for one BitSight company.
...
Each **Connect** action deploys one monitored company and creates a company-scoped DCE, a company-scoped DCR, and ten `RestApiPoller` connections, one per monitored BitSight risk vector.
...
- `bitsight-risk-findings-dce-<companyKey>`
- `bitsight-risk-findings-dcr-<companyKey>`
