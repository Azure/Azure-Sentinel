...
records to the connector's **Data Collection Endpoint** (Logs Ingestion API), so the same
DCR transform — PAN redaction and field mapping — is applied exactly as for live polling.
The full card number is never written to the workspace.

## Prerequisites

1. The solution is **installed** (this deploys the `PRODAFTUstaPCFIDCR` data collection
   rule, its data collection endpoint, and the `PRODAFTUstaCompromisedCards_CL`
   table) and the **PRODAFT USTA - Payment Card Fraud Intelligence** data connector is
...
# 1. Locate the connector's DCR (deployed by the solution as 'PRODAFTUstaPCFIDCR')
DCR_ID=$(az monitor data-collection rule list --resource-group "$RG" \
  --query "[?contains(name, 'PRODAFTUstaPCFI')].id | [0]" -o tsv)
