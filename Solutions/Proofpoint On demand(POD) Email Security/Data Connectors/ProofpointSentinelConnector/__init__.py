...
class Proofpoint_api:
    ...
    def gen_timeframe(self, time_delay_minutes):
        ...
        self.before_time = before_time.strftime("%Y-%m-%dT%H:09:59.999999")
        ...
        for attempt in range(max_retries):
            try:
                ...
                ws = websocket.create_connection(
                    url, header=header, sslopt=sslopt)
                ws.settimeout(20)
                time.sleep(2)
                logging.info(
                    'Websocket connection established to cluster_id={}, event_type={}'.format(self.cluster_id, event_type))
                print('Websocket connection established to cluster_id={}, event_type={}'.format(
                    self.cluster_id, event_type))
                return ws
            except Exception as err:
                logging.error(
                    'Error while connectiong to websocket {}'.format(err))
        ...
        if ws is not None:
            ...
            while True:
                try:
                    ...
                    logging.info("Websocket timout error")
                    break
                except Exception as err:
