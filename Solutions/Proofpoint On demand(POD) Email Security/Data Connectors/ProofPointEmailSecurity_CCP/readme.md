...
| `HMS30001` | WebSocket window timeout exceeded. The connection remained open longer than the configured `request.timeoutInSeconds` without completing. The `request.timeoutInSeconds` value is capped at 12 minutes, which is the default setting. |
