...
TABLE_NAME = "BloodHoundAttackPathsTimelineDetails_CL"
...
ENDPOINTS = {
    ...
    "attack_path_timeline": "/api/v2/domains/{domain_id}/sparkline?finding={finding_type}",
}
...
class BloodhoundManager:
    ...
    def get_available_types_for_domain(self, domain: str):
        ...
        Args:dcr-4d013aa9faa64537909486976870b9d1
    ...
    def get_attack_path_spakline_timeline(self, domain_id: str, finding_type: str):
        """
        Fetches the attack path timeline for a specific domain and finding type.
        ...
        try:
            ...
            # If no data returned or empty page, we're done
            print(f"Fetched response is {page_data}")
            return page_data
    ...
    #         while True:
    #             # Fetch a page of attack path timeline details
    #             page_data = self._api_request(
    #                 "attack_path_details",
    #                 domain_id=domain_id,
    #                 finding_type=finding_type,
    ...
    #             all_attack_path_timeline.extend(page_data)
    ...
    #         return all_attack_path_timeline

    #     except Exception as e:
    #         self._log_error(f"Error fetching attack path timeline for {domain_id} with type {finding_type}: {e}")
    #         return []
    ...
    def send_attack_data(
        ...
        log_entry = {
            "CompositeRisk": str(attack_data.get("CompositeRisk")),
...
def main():
    ...
    if response:
        ...
        print(f"unique_finding_types_data: {unique_finding_types_data}")
        ...
        for domain in domains_data:
            ...
            for attack_type in available_types:
                ...
                attack_path_timeline = (
                    bloodhound_manager.get_attack_path_spakline_timeline(
                        domain_id, attack_type
                    )
                )

                if attack_path_timeline:
                    consolidated_attack_paths_timeline.append(attack_path_timeline)
                    print(
                        f"Fetched {len(attack_path_timeline)} attack path timelines for {attack_type}."
                    )
                else:
                    consolidated_attack_paths_timeline.append([])
                    print(
                        f"Failed to fetch attack path for {attack_type} in domain {domain_name}"
                    )

        flattened_attack_paths_timeline = [
            item for sublist in consolidated_attack_paths_timeline for item in sublist
        ]

        print(f"CONSOLIDATED ATTACK PATHS TIMELINE:{flattened_attack_paths_timeline}")
        ...
        for i, attack in enumerate(flattened_attack_paths_timeline, 1):
            print(
                f"Sending attack data {i}/{len(flattened_attack_paths_timeline)}: ID {attack.get('id')}"
            )
            try:
