...
class JobStateTableStore:
    """
        ...
        This table contains a single row with {"lastTimeStamp": last_time_stamp} 
    """
    ...
    def post(self, pk: str, rk: str, data: dict = None):
        with TableClient.from_connection_string(self.connection_string, self.table_name) as table_client:
            ...
            try:
                ...
                logging.warning(
                    "Could not post entity to table %s", self.table_name)
                logging.warning(e)
                raise e
    ...
    def get(self, pk: str, rk: str):
        with TableClient.from_connection_string(self.connection_string, self.table_name) as table_client:
            try:
                logging.info(
                    "looking for %s - %s on table %s", pk, rk, self.table_name)
    ...
    def upsert(self, pk: str, rk: str, data: dict = None):
        ...
        with TableClient.from_connection_string(self.connection_string, self.table_name) as table_client:
            logging.info(
                "upserting %s - %s on table %s", pk, rk, self.table_name)
            entity_template = {
                "PartitionKey": pk,
                "RowKey": rk,
            }
    ...
    def merge(self, pk: str, rk: str, data: dict = None):
        with TableClient.from_connection_string(self.connection_string, self.table_name) as table_client:
            logging.info(
                "merging %s - %s on table %s", pk, rk, self.table_name)
            entity_template = {
                "PartitionKey": pk,
                "RowKey": rk,
            }
