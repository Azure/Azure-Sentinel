...
Write-Host("Api Request from Uri: $uri$($startDate.ToString('yyyy-MM-ddTHH:mm:ssZ'))$($filterParameters)")
...
if (-not ($response.HOST_LIST_VM_DETECTION_OUTPUT.RESPONSE.HOST_LIST -eq $null))
{
    ...
    $Detectionsjson = ConvertTo-Json $customObjects
    Write-Host("Retrieved Detections in json: $Detectionsjson")
	...
	$customObjects | ForEach-Object {  
        $records = $_ | ConvertTo-Json -Compress -Depth 
        Write-Host("Detection in json: $records")
