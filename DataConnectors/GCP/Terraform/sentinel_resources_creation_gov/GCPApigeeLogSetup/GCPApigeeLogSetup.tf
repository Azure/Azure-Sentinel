...
variable "topic-name" {
  type        = string
  default     = "sentinelGcpApigee-topic"
...
resource "google_pubsub_topic" "sentinelGcpApigee-topic" {
  count   = "${var.topic-name != "sentinelGcpApigee-topic" ? 0 : 1}"
  name    = var.topic-name
  project = data.google_project.project.project_id
}
