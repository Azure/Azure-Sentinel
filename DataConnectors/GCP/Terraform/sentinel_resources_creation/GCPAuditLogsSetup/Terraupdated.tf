 terraform {
    required_providers {
      google = {
        source  = "hashicorp/google"
        version = "3.73.0"
      }
    }
  
    required_version = ">= 0.15.0"
  }
  
  data "google_project" "project" {}
  
  # Variable to specify the log project where logs should be stored
  variable "log-project-id" {
    type    = string
    default = "log-project"  # Set the log project ID here (where you want the logs to go)
  }
  
  variable "topic-name" {
    type    = string
    default = "sentinel-topic"
    description = "Name of existing topic"
  }
  
  variable "organization-id" {
    type    = string
    default = ""
    description = "Organization id"
  }
  
  # Enable the logging API in the log project
  resource "google_project_service" "enable-logging-api" {
    service = "logging.googleapis.com"
    project = var.log-project-id
  }
  
  # Create the Pub/Sub topic in the log project
  resource "google_pubsub_topic" "sentinel-topic" {
    count  = "${var.topic-name != "sentinel-topic" ? 0 : 1}"
    name   = var.topic-name
    project = var.log-project-id
  }
  
  # Create the Pub/Sub subscription in the log project
  resource "google_pubsub_subscription" "sentinel-subscription" {
    project = var.log-project-id  # Use the log project ID here
    name    = "sentinel-subscription-auditlogs"  # Only the subscription name
    topic   = var.topic-name
    depends_on = [google_pubsub_topic.sentinel-topic]
  }
  
  # Logging sink for the project (logs are stored in log project)
  resource "google_logging_project_sink" "sentinel-sink" {
    project    = var.log-project-id  # Use the log project ID here
    count      = var.organization-id == "" ? 1 : 0
    name       = "audit-logs-sentinel-sink"
    destination = "pubsub.googleapis.com/projects/${var.log-project-id}/topics/${var.topic-name}"
    depends_on = [google_pubsub_topic.sentinel-topic]
    unique_writer_identity = true
  }
  
  # Logging sink for the organization (logs are stored in log project)
  resource "google_logging_organization_sink" "sentinel-organization-sink" {
    count      = var.organization-id == "" ? 0 : 1
    name       = "audit-logs-organization-sentinel-sink"
    org_id     = var.organization-id
    destination = "pubsub.googleapis.com/projects/${var.log-project-id}/topics/${var.topic-name}"
    include_children = true
  }
  
  # IAM binding for log writer (using log project)
  resource "google_project_iam_binding" "log-writer" {
    count    = var.organization-id == "" ? 1 : 0
    project  = var.log-project-id  # Use the log project ID here
    role     = "roles/pubsub.publisher"
    members  = [
      google_logging_project_sink.sentinel-sink[0].writer_identity
    ]
  }
  
  # IAM binding for log writer at the organization level (using log project)
  resource "google_project_iam_binding" "log-writer-organization" {
    count    = var.organization-id == "" ? 0 : 1
    project  = var.log-project-id  # Use the log project ID here
    role     = "roles/pubsub.publisher"
    members  = [
      google_logging_organization_sink.sentinel-organization-sink[0].writer_identity
    ]
  }
  
  output "An_output_message"{
    value = "Please copy the following values to Sentinel"
  }
  
  output "GCP_project_id" {
    value       = data.google_project.project.project_id
  }
  
  output "GCP_project_number" {
    value       = data.google_project.project.number
  }
  
  output "GCP_subscription_name" {
    value       = google_pubsub_subscription.sentinel-subscription.name
  }
  