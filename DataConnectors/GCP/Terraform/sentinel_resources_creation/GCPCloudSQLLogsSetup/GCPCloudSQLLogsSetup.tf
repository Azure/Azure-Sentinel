...
resource "google_pubsub_topic" "sentinelcloudsql-topic" {
  count   = "${var.topic-name != "sentinelcloudsql-topic" ? 0 : 1}"
  name    = var.topic-name
  project = data.google_project.project.project_id
}
