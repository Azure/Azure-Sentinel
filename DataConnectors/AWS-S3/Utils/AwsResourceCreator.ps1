...
@ -11,126 +10,120 @@ function New-OidcProvider
...
@ -140,59 +133,64 @@ function New-S3Bucket
    Set-RetryAction(
        {
        ...
        if ($isBucketNotExist)
        {
            ...
            {     
                ...
                if ($bucketCreationConfirm -eq 'y') {     
                    if ($regionConfiguration -eq "us-east-1") {
                        # see aws doc https://docs.aws.amazon.com/cli/latest/reference/s3api/create-bucket.html
                        Write-Log -Message "Executing: aws s3api create-bucket --bucket $bucketName 2>&1" -LogFileName $LogFileName -Severity Verbose
                        $tempForOutput = aws s3api create-bucket --bucket $bucketName 2>&1
                        Write-Log -Message $tempForOutput -LogFileName $LogFileName -Severity Verbose
                    }
...
@ -200,34 +198,51 @@ function New-S3Bucket
...
@ -236,100 +251,111 @@ function Enable-S3EventNotification
