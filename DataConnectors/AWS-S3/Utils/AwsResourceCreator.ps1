function New-ArnRole
{
    ...
    Set-RetryAction({
        $script:roleName = Read-Host 'Please enter role name. If you have already configured an Assume Role for Azure Sentinel, use the same role name'
        aws iam get-role --role-name $roleName 2>&1| Out-Null
        $isRuleNotExist = $lastexitcode -ne 0
        if ($isRuleNotExist)
        {
            
            Write-Output "`n`nYou must specify the the Azure Sentinel Workspace ID. This is found in the Azure Sentinel portal."
            $workspaceId = ""
            while ($workspaceId -eq "")
            {
                $workspaceId = Read-Host 'Please enter your Azure Sentinel Workspace ID (External Id)'
            }
