...
Retry-Action( {
		$eventNotificationName = Read-Host 'Please enter Vpc Event Notifications Name'
		...
		if ($prefixOverrideConfirm -eq 'y') {
			$eventNotificationPrefix = Read-Host 'Please enter Vpc Event Notifications Prefix'
		}
