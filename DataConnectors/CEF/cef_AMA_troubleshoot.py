...
def print_result_to_prompt(is_successful, result_text):
    """
    Printing the test's name and success status to the customer's prompt
    """
    max_length = 47
    if is_successful == "Warn":
        print_warning(result_text + "-" * (max_length - len(result_text)) + "> Failed to check")
    elif is_successful:
        print_ok(result_text + "-" * (max_length - len(result_text)) + "> Success")
    else:
        print_error(result_text + "-" * (max_length - len(result_text)) + "> Failure")
...
class BasicCommand:
    ...
    def __init__(self, command_name, command_to_run, result_keywords_array=None, fault_keyword=None,
                 ...
                 is_successful=False):
        if result_keywords_array is None:
            result_keywords_array = []
    ...
    def document_result(self):
        ...
        """
        print_result_to_prompt(self.is_successful, self.command_name)
...
class DCRConfigurationVerifications:
    ...
    def verify_DCR_exists(self):
        ...
        command_object.run_full_test()
        if not command_object.is_successful:
            print_error(self.DCR_missing_error_messgae)
            return False
        return True
    ...
    def verify_DCR_content_has_CEF_stream(self):
        ...
        command_object.run_full_test()
        if not command_object.is_successful:
            print_error(self.DCR_missing_CEF_stream_error_message)
            return False
        return True
...
class SyslogDaemonVerifications:
    ...
    def verify_Syslog_daemon_listening(self):
        ...
        if command_object.is_successful is not False:
            ...
            if not command_object.is_successful:
                print_warning(self.Syslog_daemon_not_listening_warning(self.SYSLOG_DAEMON))
    ...
    def verify_Syslog_daemon_forwarding_configuration(self):
        ...
        if self.SYSLOG_DAEMON != "":
            ...
            command_object.run_full_test()
            if not command_object.is_successful:
                print_error(self.Syslog_daemon_not_forwarding_error(self.SYSLOG_DAEMON))
    ...
    def run_all_verifications(self):
        ...
        if self.determine_Syslog_daemon():
            ...
            print_result_to_prompt(False, self.command_name)
...
class SystemInfo:
    ...
    def __repr__(self):
        delimiter = "\n" + "-" * 20 + "\n"
        return str(
            delimiter + "command: " + str(self.command_object.command_name) + '\n' + "output: " + str(
                self.command_object.command_result) + delimiter).replace(
            '%', '%%').replace('\\n', '\n')

    def append_content_to_file(self, file_path=COLLECT_OUTPUT_FILE):
        """
        ...
        try:
            ...
            print(str(self.command_object.command) + "was not documented successfully")
    ...
    def handle_commands(self):
        ...
        for command_name in self.commands_dict.keys():
            self.command_object = BasicCommand(command_name, self.commands_dict[command_name])
            self.command_object.run_command()
