...
function QualysKB {
    ...
    Write-Host "Logging in with credentials using URI: $base/session/"
    ...
    Write-Host "Invoking API Request URI: $base/knowledge_base/vuln/?action=list&published_after=$($startDate)$filterparameters"
    ...
    Write-Host "Object length from response: $objsLength"
    if ($objs.Length -ne 0){
