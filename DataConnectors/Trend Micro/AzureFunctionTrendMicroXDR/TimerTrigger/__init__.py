...
def getWorkbench(workbencheIds):
    url_path1 = '/v2.0/xdr/workbench/workbenches/'+workbencheIds
    url1 = url_base+url_path1
    headers = {'Authorization': 'Bearer ' + api_id, 'Content-Type': 'application/json;charset=utf-8'}
...
def customize_json(json2_data):
    for impact_scope in json2_data['impactScope']:
        ...
        if simple_str_field not in json2_data:  # use first element only
            json2_data[simple_str_field] = scope_value
        if entity_type == 'host':
            json2_data['impactScope_hostname'] = hostname
            json2_data['impactScope_hostGuid'] = host_guid

        comma_seperated_field = 'impactScope_' + entity_type + 's'
        if comma_seperated_field not in json2_data:
            json2_data[comma_seperated_field] = scope_value
        else:
            json2_data[comma_seperated_field] = "{}, {}".format(json2_data[comma_seperated_field], scope_value)
...
def function():
    ...
    if "totalCount" in workbench_list['data']:
        ...
        print(workbenchcount)
        
        while a < workbenchcount:
         workbencheIds = workbench_list['data']['workbenchRecords'][a]['workbenchId']
         workbench_details = getWorkbench(workbencheIds)
         ...
         #print (body)
         post_data(customer_id, shared_key, body, log_type, workbencheIds)
         a += 1
