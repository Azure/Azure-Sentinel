...
Function Get-Watchlists() {
    ...
    try {
        ...
        $responseWatchlist = (Invoke-RestMethod -Method Get -Headers $requestHeaders -Uri $apiWatchlist).value
    }
    catch {
    ...
    Write-Host "The watchlists with following aliases are found:"
    $aliases = $responseWatchlist | ForEach-Object { $_.properties.watchlistAlias }
    Write-Host ($aliases -join ", ")
