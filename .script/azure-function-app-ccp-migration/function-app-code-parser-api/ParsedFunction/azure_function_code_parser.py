...
bing_key = (
    ...
    or os.getenv("BING_SEARCH_V7_SUBSCRIPTION_KEY")
)
searcher = BingWebSearcher(subscription_key=bing_key)
...
def fetch_api_urls(search_query: str, top: int = 15) -> List[str]:
    """Run Bing search and return concatenated snippets (url + text)."""
    urls = searcher.get_search_result_links(search_query, top=top)
    return urls
