...
let errorMessage = "";
export function isValidLogoImageSVGContent(LogoImagesContent) {
...
;
function isLogoSVGHasillegalAttribute(LogoImagesContent) {
...
function isLogoSVGHasTitleTag(LogoImagesContent) {
    return LogoImagesContent.includes("<title>");
}
function isLogoSVGHasPNGEmbed(LogoImagesContent) {
    if (LogoImagesContent.includes("<image")) {
        return LogoImagesContent.includes(".png");
    }
    return false;
}
function isValidGuid(guidId) {
    const validRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
    return validRegex.test(guidId);
}
function isLogoSVGHasValidId(LogoImagesContent) {
    var regex = /id=\"/g;
...
//# sourceMappingURL=logoImageSVGChecker.js.map