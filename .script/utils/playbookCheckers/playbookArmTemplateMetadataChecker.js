...
import { PlaybookMetadataSupportedEntityTypes } from "./Models/playbookTemplateMetadata.js";
import { isNullOrWhitespace, isPlaybookUsingGalleryMetadata } from "./playbookARMTemplateUtils.js";
export function validateTemplateMetadata(filePath, playbookARMTemplate) {
...
function validateMetadataMandatoryFieldsAreNotEmpty(filePath, playbookARMTemplate) {
...
function validateMetdataLastUpdateTimeIsValidTimestamp(filePath, playbookARMTemplate) {
    let parsedLastUpdateTime = new Date(playbookARMTemplate.metadata.lastUpdateTime);
...
function validateMetdataEntitiesContainsValidEntityTypes(filePath, playbookARMTemplate) {
    if (!playbookARMTemplate.metadata.entities) {
        return;
    }
    let invalidEntityTypes = playbookARMTemplate.metadata.entities
        .filter((entityType) => !PlaybookMetadataSupportedEntityTypes.includes(entityType.toLowerCase()));
...
//# sourceMappingURL=playbookArmTemplateMetadataChecker.js.map